...
import xyz.xuminghai.test.VolatileTest;
import org.openjdk.jcstress.infra.results.I_Result;

public final class VolatileTest_jcstress extends Runner<I_Result> {

    volatile WorkerSync workerSync;
    VolatileTest[] gs;
    I_Result[] gr;

    public VolatileTest_jcstress(ForkedTestConfig config) {
        super(config);
    }
    ...
    private void sanityCheck_Footprints(Counter<I_Result> counter) throws Throwable {
        config.adjustStrideCount(new FootprintEstimator() {
          public void runWith(int size, long[] cnts) {
            ...
            VolatileTest[] ls = new VolatileTest[size];
    ...
    public ArrayList<CounterThread<I_Result>> internalRun() {
        ...
        gs = new VolatileTest[len];
        gr = new I_Result[len];
        for (int c = 0; c < len; c++) {
            gs[c] = new VolatileTest();
            gr[c] = new I_Result();
        }
    ...
    private void run_actor1(VolatileTest[] gs, I_Result[] gr, int start, int end) {
        VolatileTest[] ls = gs;
        I_Result[] lr = gr;
    ...
    private void run_actor2(VolatileTest[] gs, I_Result[] gr, int start, int end) {
        VolatileTest[] ls = gs;
        I_Result[] lr = gr;
        for (int c = start; c < end; c++) {
            VolatileTest s = ls[c];
